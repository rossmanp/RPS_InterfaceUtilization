...
namespace RockPaperScissors
{
    class RoshamboApp
    {
        ...
        public static int DetermineWinner(IPlayer computerPlayer, string name)
        {
            User human = new User(name);
            Roshambo userRoshambo = human.generateRoshambo();
            Roshambo computerRoshambo = computerPlayer.generateRoshambo();
            Console.WriteLine("\n" + name + ": " + userRoshambo);
            Console.WriteLine(computerPlayer.getName() + ": " + computerRoshambo);
            ...
            {
                Console.WriteLine(computerPlayer.getName() + " wins!");
                return -1;
            }
            else
            {
